...
import ru.javacourse.blog.dao.impl.CommentDao;
...
public class BlogServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!isNullOrEmpty(commentText) && !isNullOrEmpty(commentAuthor)) {
            Post post = postDao.getById(Integer.parseInt(postId));

            Comment comment = new Comment(commentText, new Date(), commentAuthor, post);
            commentDao.create(comment);
           // response.setAttribute("post", post);
            response.sendRedirect("/blog?post=" + post.getId());
        } else if (isValidPost(title, summary, body)){
